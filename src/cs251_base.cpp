...
/*! \fn base_sim_t::base_sim_t 
    \brief This is the constructor for the base_sim_t class
*/
...
/*! \fn base_sim_t::~base_sim_t
    \brief This is the destructor for the base_sim_t class
*/
base_sim_t::~base_sim_t()
{
...
/*! \fn base_sim_t::draw_title
    \brief This function draws the title on the left of the screen
*/ 
...
/*! \fn base_sim_t::step
    \brief This is the function that calculates the physical properties of the elments after every time step.
*/ 
void base_sim_t::step(settings_t* settings)
{
  ...
  //cout<<"ABCDEF"<<endl;
	if(clock_tart == true)
	{
	 float timeElapsed = ((float)( clock() - clk )) / CLOCKS_PER_SEC;
	if(timeElapsed >= 5.0f){
	   ba->SetType(b2_staticBody);
           
	}	
	}
  /// We have maintained a boolean public data member of the base_sim_t class called m_contact which is initially false and changes to true when there is contact between the required bodies (Catenary curve) and the balloon, and hence it destroys the joint then.
  if (m_contact == true){
  //  cout<<"ABC"<<endl;
    ...
    b2Joint* j1 = je->joint;
    //cout<<"Hello"<<endl;
    //if ((j1->GetReactionForce(time_step).LengthSquared() > 0.2f)){
    m_world->DestroyJoint(j1);
    b2Vec2 vel(0.0f, -1.0f);
    clk = clock();
    clock_tart = true;
    ba->SetLinearVelocity(vel);
    m_world->DestroyBody(mb4);
    }
//      cout<<"DEFGH"<<endl;
  }
  ...
  if(s_contact==true)
  {
    m_world->DestroyBody(sbody1);
    m_world->DestroyBody(ground2);
    s_contact = false;
  }
